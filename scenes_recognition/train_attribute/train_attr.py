...
def parse_args():
    ...
    parser.add_argument('--solver', dest='solver',
                        help='solver',
                        default='solver_attr_MobileNetV2.prototxt', type=str)
...
class SolverWrapper(object):
    ...
    def __init__(self, solver_prototxt,
        ...
        self.errors = []                 
        self.errorsTrain = []
        if os.path.exists('./tmp/errors.npy'):
            self.errors = np.load('./tmp/errors.npy').tolist()
        if os.path.exists('./tmp/errorsTrain.npy'):    
    ...
    def drawErrors(self):
        ...
        ymin = np.min(self.errorsTrain,axis = 0)[1]
        ymax = np.max(self.errorsTrain,axis = 0)[1]
        plt.ylim(ymin, ymax)
    ...
    def train_model(self):
        ...
        while self.solver.iter < max_iters:
            ...
            if self.solver.iter % self.solver_param.display == 0:
                ...
                self.errorsTrain.append((self.solver.iter, train_loss))
                train_loss_single = 0
            ...
            if self.solver.iter % self.solver_param.test_interval == 0:
                ...
                self.errors.append((self.solver.iter, test_loss))
                test_loss_single = 0
...
def train_net(solver_prototxt,
    ...
    print 'done solving'
    net = sw.solver.net
    filename = ('attr_latest' + '.caffemodel')
    filename = os.path.join(os.getcwd, filename)
    net.save(str(filename))
    print 'Wrote latest to: {:s}'.format(filename)
...
if __name__ == '__main__':
    ...
    pretrained_model = '/home/beauty/project/places365-caffe/atti_train/model/MobileNetV2_attr__iter_15600.solverstate'
